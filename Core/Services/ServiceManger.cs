...
namespace Services
{
    public class ServiceManger(IUnitOfWork unitOfWork , IMapper mapper ,
        ...
        , ITokenRepository _tokenRepository , UserManager<ApplicationUser> userManager , SignInManager<ApplicationUser> signInManager) 
        : IServiceManger
    {
        ...
        private Lazy<IUserServices> _userServices = new Lazy<IUserServices>(() => new UserServices(userManager,signInManager, _tokenServices.Value));

        public IUserServices UserServices => throw new NotImplementedException();
    }
}
